...
installEssential(){
    ...
    dict=( nginx ssh wget vim dkms build-essential module-assistant dos2unix net-tools git wireless-tools linux-headers-$(uname -r) libgtk2.0-0 openjdk-11-jdk ant duplicity python-gi)
#    for i in "${dict[@]}"
#    do
#        infoLog "$i"
#    done
...
#sleep 10
	infoLog "Installing `echo ${dict[@]}`"
	apt-get install -y `echo ${dict[@]}`
	infoLog "Installing `echo ${dict[@]}` complete!!!"
