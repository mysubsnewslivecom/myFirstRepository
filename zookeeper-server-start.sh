    ...
    if [ -f "$PIDFILE" ]; then
      if [ -s "$PIDFILE" ]; then
        ...
        statusService "$PID" "${application}"
              ...
              nohup ${base_dir}/kafka-run-class.sh $EXTRA_ARGS \org.apache.zookeeper.server.quorum.QuorumPeerMain "${PROPERTY_FILE}"  > /dev/null  2>&1 < /dev/null &
                echo $! > ${PIDFILE}
                local PID=`cat ${PIDFILE}`
...
#	echo "USAGE: $0 [-daemon] zookeeper.properties"
    PROPERTY_FILE="$KAFKA_CONF/zookeeper.properties"
fi
base_dir=$(dirname $0)
...
#nohup ${base_dir}/kafka-run-class.sh $EXTRA_ARGS \
